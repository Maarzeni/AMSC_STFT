 ...
 *
 * @details
 * Both call paths must be bit-identical: operator() delegates to apply(),
 * so there must be no divergence in computed values.
 */
TEST_F(HannWindowTest, OperatorCallMatchesApply) {
    HannWindow w(MEDIUM_SIZE);
